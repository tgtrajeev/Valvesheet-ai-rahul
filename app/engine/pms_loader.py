...
@dataclass
class PmsMaterials:
    """Material grades for the valve, sourced from the project's PMS document.

    Per ASME B16.34 §6.1, the body/bonnet/closure material is selected from the
    appropriate Group (1=ferrous, 2=austenitic, 3=nickel-alloy). The specific
    ASTM grade is fixed by the project's piping-material spec — that's what
    these fields hold. Trim grades follow API 615 §6.2 ("trim corrosion
    resistance ≥ body").
    """
    spec_code: str
    body_material: str | None = None      # e.g., "ASTM A105N (≤1.5"), ASTM A216 WCB (≥2")"
    ball_material: str | None = None      # ball/disc/wedge/needle obturator material
    stem_material: str | None = None
    seat_material: str | None = None      # for metal-seated; soft seats come from valve type rule
    gland_material: str | None = None
    gland_packing: str | None = None
    spring_material: str | None = None
    lever_handwheel: str | None = None
    asme_b1634_group: int | None = None   # 1, 2, or 3 — the group that drives P-T tables
...
class PmsLoader:
    ...
    def _parse_spec(self, code: str, data: dict) -> PmsSpec:
        ...
        if idx_raw:
            ...
            # Backend-sourced specs use pt_ratings + header instead of index_row.
            # Synthesize an index_row so the Rule Engine can resolve design_pressure
            # and hydrotest using the same code path as Excel-extracted specs.
            ...
            if pt:
                ...
                breakpoints = [
                    ...
                    for r in pt[1:]  # remaining rows become the breakpoint table
                ]
                index_row = PmsIndexRow(
                    spec_code=code,
    ...
    def get_spec(self, spec_code: str) -> PmsSpec | None:
        ...
        if spec is None:
            spec = self._fetch_from_backend(code)
        return spec

    def refresh_spec_from_backend(self, spec_code: str) -> PmsSpec | None:
        """Fetch the latest version of one spec from the backend, replacing cache.

        This is used before datasheet generation so an edited PMS class changes
        the next generated sheet without requiring a chatbot restart.
        """
        code = spec_code.upper().strip()
        return self._fetch_from_backend(code) or self._specs.get(code)

    def _preload_from_backend(self) -> None:
        """Fetch all classes from GET /api/pms/classes and add any not already loaded."""
        try:
            import httpx
            ...
            for cls in resp.json().get("classes", []):
                code = (cls.get("spec_code") or "").upper().strip()
                if not code or code in self._specs:
                    continue
                # Build a lightweight PmsSpec from the class summary header
                header = PmsHeader(
                    spec_code=code,
                    pressure_rating=cls.get("pressure_rating"),
                    material_description=cls.get("material"),
                    corrosion_allowance=cls.get("corrosion_allowance"),
                    design_code=None,
                    service=cls.get("service"),
                    nace_flag=bool(cls.get("nace", False)),
                    lt_flag=bool(cls.get("low_temp", False)),
                    design_pressure_barg=cls.get("design_pressure_barg"),
                    hydrotest_pressure_barg=cls.get("hydrotest_pressure_barg"),
                )
                self._specs[code] = PmsSpec(
                    spec_code=code,
                    header=header,
                    bolting_gaskets=None,
                    flanges=[],
                    index_row=None,
                    valve_assignments=[],
                    pt_ratings=[],
    ...
    def _fetch_from_backend(self, spec_code: str) -> "PmsSpec | None":
        """Fetch a single PMS spec from the backend API and cache it locally."""
        try:
            import httpx
            from ..config import settings
            resp = httpx.get(
            ...
            if resp.status_code == 200:
                payload = resp.json()
                sheet = payload.get("data") or {}
                if sheet:
                    parsed = self._parse_spec(spec_code, sheet)
                    self._specs[spec_code] = parsed
                    return parsed
        except Exception:
            pass
        return None
    ...
    def get_hydrotest(self, spec_code: str) -> tuple[float | None, float | None]:
        """Return (shell_barg, closure_barg) — matches rule_engine PMS hydrotest logic."""
    ...
    def get_design_pressure(self, spec_code: str) -> float | None:
        ...
        if not spec:
            return None
        if spec.index_row and spec.index_row.design_pressure_barg:
            return spec.index_row.design_pressure_barg
        return spec.header.design_pressure_barg
...
def get_pms_loader() -> PmsLoader:
    ...
    if _loader is None:
        ...
        if pms_path.exists():
            ...
            _loader = PmsLoader.__new__(PmsLoader)
            _loader._specs = {}
        # Preload any classes synced to the backend (fills gaps beyond local file)
        _loader._preload_from_backend()
    return _loader


def refresh_pms_loader() -> None:
    """Force a full reload of the PMS loader (call after new PMS is synced)."""
    global _loader
    _loader = None
    get_pms_loader()
